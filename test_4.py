        ...
        def wrapper(*args, **kwargs):
            ...
            while attempts < n:
                try:
                    ...
                    if attempts == n:
                        raise e
                    time.sleep(1)  # 可以根据需要调整重试间隔
            return None
        return wrapper
    return decorator

@retry(5)
def func(m: class163.Music):
    m.__init__(m.id)
    ...
    with open(f"{direeee}/{m.title} - {artist_join(m.artist, ", ")}.mp3", mode="wb") as f:
        f.write(music_file_bytes)
    with open(f"{direeee}/{m.title} - {artist_join(m.artist, ", ")}.lrc", mode="w", encoding="UTF-8") as f:
        if m.lyric != None:
            f.write(m.lyric)
    print(f"{m.title} - {artist_join(m.artist, ", ")} OK")
    m.music_file.data = None
    m.cover_file.data = None


direeee = str(input("dir: "))
